...
- (void)setDataSource:(id<DOPDropDownMenuDataSource>)dataSource {
    ...
    for (int i = 0; i < _numOfMenu; i++) {
        ...
        CALayer *bgLayer = [self createBgLayerWithColor:[UIColor purpleColor] andPosition:bgLayerPosition];
        ...
        //indicator  titlePosition.x + title.bounds.size.width / 2 + 8
        CAShapeLayer *indicator = [self createIndicatorWithColor:self.indicatorColor andPosition:CGPointMake((CGRectGetMaxX(bgLayer.frame) - 8), self.frame.size.height / 2)];
...
//    CAShapeLayer *indicator = (CAShapeLayer *)_indicators[_currentSelectedMenudIndex];
    //title.position.x + title.frame.size.width / 2 + 8
//    indicator.position = CGPointMake((CGRectGetMaxX(self.bgLayers.frame) - 5), indicator.position.y);
}
